...
def run_trading_comps(target_company: str):
    ...
    Path("data/comparables/processed").mkdir(parents=True, exist_ok=True)
    Path("outputs/comparables/tables").mkdir(parents=True, exist_ok=True)

    if not re.fullmatch(r"[A-Z0-9.\-]{1,10}", target_company):
        raise ValueError("target_company must look like an equity ticker (e.g., 'NVDA', 'BRK.B').")
    ...
    required = {
        "market_data": comps_market_data_df,
        "net_debt": comps_net_debt_df,
        "financials": comps_financials_df
    }

    for name, df in required.items():
        df.set_index("ticker", inplace=True)
        if target_company not in df.index:
            raise KeyError(f"Target company '{target_company}' not found in {name}.")
    ...
    if not combined_df.index.is_unique:
        raise ValueError("Merged comps index is not unique (duplicate tickers).")

    if combined_df.shape[0] != len(comps_market_data_df):
        raise ValueError("Row count mismatch after merge (likely missing tickers in net debt or financials).")
    ...
    implied_ev = pd.Series(
        {"min": ev_ebitda_min, "mean": ev_ebitda_mean, "median": ev_ebitda_median, "max": ev_ebitda_max}
    ) * target_ebitda
    implied_equity = implied_ev - target_net_debt
    implied_per_share = implied_equity / target_shares

    target_implied_valuation = pd.DataFrame(
        {
